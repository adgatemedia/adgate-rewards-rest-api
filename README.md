...
You may fetch offers for mobile devices by passing a `device` GET param with a value of `android`, `iphone`, or `ipad`. If your users are coming from a native app, pass a `android_id` or `ios_id` GET variable with the corresponding unique device ID provided by the app. This will return offers that require these identifiers to be preset.
